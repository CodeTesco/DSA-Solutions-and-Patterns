...
The reason being the that algorithm searches for a target within the bounds of [left, right] with mid = (left + right // 2). If the number is target >= nums[mid], then we know the target cannot be in any of the indexes <= left so we increment left by 1 (left + 1). Likewise if the target <= nums[mid], then we know that the number can't be in any of the indexes >= right so we decrement right by 1 (right + 1).
